...
// 🟢 STEP 2: YOUR FIREBASE CONFIGURATION (Already Pre-Filled) 🟢
...
/* ==========================================
  HELPER: TEXT FORMATTER (Markdown -> JSX)
  ==========================================
*/
...
const generateAIQuiz = async (topic, currentMastery, dbInstance) => {
  let promptData = null;
  let examplesText = '';
  ...
  const difficultyLabel =
    difficultyTier === 'level1'
      ? 'Foundational/Easy'
      : difficultyTier === 'level2'
      ? 'Medium/Standard'
      : 'Hard/Advanced';
  ...
  try {
    ...
    if (!snapshot.empty) {
      ...
      const tieredExamples =
        customData[`examples_${difficultyTier}`] || customData.examples || [];
      promptData = {
        basePrompt: customData.basePrompt,
        examples: tieredExamples,
      };
    }
  } catch (e) {
  ...
  if (promptData && promptData.examples.length > 0) {
    examplesText = promptData.examples
      .map(
        (ex, i) => `
    REF ${i + 1}: Question: "${
          ex.question ? ex.question.substring(0, 50) : ''
        }..." | Correct: ${ex.correct} | Trap: ${ex.logic}`
      )
      .join('\n');
  }
...
const generateDiagnosticBatch = async (subject, distribution) => {
  const systemPrompt = `
      You are an expert SAT Diagnostic creator for ${subject}.
      GOAL: Create exactly ${
        distribution.length
      } multiple-choice questions based on this distribution:
      ${distribution.join(', ')}
...
const generateFullDiagnostic = async () => {
  ...
  try {
    ...
    if (!Array.isArray(mathQs) || !Array.isArray(readQs))
...
const generateTutorResponse = async (context, userMessage) => {
  const systemPrompt = `
    ...
    Correct Answer: Option ${String.fromCharCode(
      65 + context.correctIndex
    )} ("${context.options[context.correctIndex]}")
    Student Selection: Option ${String.fromCharCode(
      65 + context.selectedOption
    )} ("${context.options[context.selectedOption]}")
...
const TeacherView = ({ onLogout }) => {
  ...
  const [examplesL1, setExamplesL1] = useState([]);
  const [examplesL2, setExamplesL2] = useState([]);
  const [examplesL3, setExamplesL3] = useState([]);
  const [showPicker, setShowPicker] = useState(false);

  useEffect(() => {
    const q = query(
      collection(db, 'artifacts', appId, 'public', 'data', 'prompt_templates')
    );
  ...
  const pickOfficialSkill = (domain, skill) => {
  ...
  const setCurrentExamples = (newEx) => {
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8">
          {isEditing ? (
            <div className="max-w-3xl mx-auto space-y-6">
              <div className="flex justify-between items-center">
                <h1 className="text-2xl font-black text-slate-800">
                  {selectedSkill ? 'Edit Skill Logic' : 'New Skill Logic'}
                </h1>
              ...
              {showPicker && (
                <div className="bg-indigo-50 border border-indigo-100 p-4 rounded-xl mb-6">
                  <h3 className="text-sm font-bold text-indigo-800 mb-3 flex items-center gap-2">
                    <BookOpen size={16} /> Select Official Skill to Autofill
                  </h3>
                      ...
                      Object.entries(domains).map(([domain, skillList]) => (
                        <div key={domain}>
                          ...
                          {skillList.map((skill) => (
                            <button
                              key={skill}
                              onClick={() => pickOfficialSkill(domain, skill)}
                              className="block w-full text-left text-sm py-1 px-2 hover:bg-white rounded text-slate-600 hover:text-indigo-600 transition"
                            >
                              {skill}
                            </button>
                          ))}
              ...
              <div>
                ...
                <textarea
                  className="w-full p-4 h-32 bg-slate-900 text-emerald-400 font-mono text-sm rounded-lg outline-none"
                  value={promptInput}
              ...
              <div className="space-y-4">
                <div className="flex justify-between items-end border-b pb-2">
                    ...
                    <div className="flex gap-2">
                      {['level1', 'level2', 'level3'].map((lvl) => (
                        <button
                          ...
                          className={`px-3 py-1 rounded-t-lg text-sm font-bold flex items-center gap-2 ${
                            activeTab === lvl
                  ...
                  <button
                    ...
                    className="text-xs flex items-center gap-1 text-indigo-600 font-bold hover:underline mb-2"
                  >
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                          <div className="col-span-1 md:col-span-2">
                            ...
                            <input
                              ...
                              value={ex.question}
                              onChange={(e) =>
                                updateExample(idx, 'question', e.target.value)
                              }
                            />
                          </div>
                          <div>
                            ...
                            <input
                              ...
                              value={ex.correct}
                              onChange={(e) =>
                                updateExample(idx, 'correct', e.target.value)
                              }
                            />
                          </div>
                          <div>
                            ...
                            <input
                              className="w-full p-2 text-sm border rounded bg-slate-50 text-red-600"
                              value={ex.logic}
                              onChange={(e) =>
                                updateExample(idx, 'logic', e.target.value)
                              }
                            />
                          </div>
                        </div>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-400 space-y-4">
              ...
              <p>
                Select a skill from the left to edit its AI logic, or create a
                new one.
              </p>
              <button
                onClick={handleNewSkill}
                className="md:hidden p-3 bg-indigo-600 text-white rounded-lg font-bold"
              >
                Create New Skill
              </button>
            </div>
...
const DiagnosticReportView = ({ reportData, onConfirm }) => {
  ...
  const focusStats = priorities.find((p) => p.domain === primaryFocus);
  const focusAccuracy = focusStats ? Math.round(focusStats.accuracy * 100) : 0;
  const focusWeight = DOMAIN_WEIGHTS[primaryFocus]
    ? DOMAIN_WEIGHTS[primaryFocus] * 100
    : 0;
    ...
    <div className="max-w-2xl mx-auto bg-slate-50 min-h-screen p-6 flex flex-col">
      <div className="text-center mb-8 animate-in slide-in-from-top-4">
        ...
        <p className="text-slate-500">
          We have analyzed your performance profile.
        </p>
      </div>
          ...
          <div className="flex items-start gap-4">
            ...
            <div>
              ...
              <p className="text-slate-600 text-sm mt-1 leading-relaxed">
                Your accuracy in this domain was{' '}
                <strong>{focusAccuracy}%</strong>. Since this domain makes up
                roughly <strong>{focusWeight}%</strong> of the total SAT score,
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 mb-8 animate-in slide-in-from-bottom-4 delay-200">
        <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
          <Activity size={18} className="text-slate-400" /> Performance
          Breakdown
        </h3>
        <div className="space-y-3">
          {priorities.map((p, i) => (
            <div
              key={i}
              className="flex items-center justify-between p-2 hover:bg-slate-50 rounded-lg transition"
            >
...
const DiagnosticView = ({ onComplete, onSkip }) => {
  ...
  const loadQuestions = async () => {
    ...
    if (q && Array.isArray(q) && q.length > 0) {
      ...
      setError(
        'Failed to generate diagnostic test. Please check your connection and try again.'
      );
    }
    setLoading(false);
  };
  ...
  const handleAnswer = (index) => {
    ...
    const newAnswers = {
      ...answers,
      [currentIndex]: { correct: isCorrect, domain: currentQ.domain },
    };
  ...
  const calculateResults = (finalAnswers) => {
    let domainScores = {};
    Object.keys(DOMAIN_WEIGHTS).forEach(
      (d) => (domainScores[d] = { correct: 0, total: 0 })
    );
  ...
  if (loading)
    return <Spinner text="Constructing 16-Question Diagnostic Battery..." />;
      ...
      <div className="bg-white p-6 rounded-2xl shadow-xl border border-slate-200">
        <div className="flex items-center justify-between mb-6">
          ...
          <div className="flex items-center gap-3">
            <div
              className={`flex items-center gap-1 font-mono font-bold px-2 py-1 rounded ${
                timeLeft < 120
...
const StudentView = ({ user, onLogout }) => {
  ...
  const handleCreateProfile = async (name, email) => {
    if (!user) return;
  ...
  const submitAnswer = async (index) => {
    ...
    const isMath =
      profile.focusArea &&
      (profile.focusArea.includes('Math') ||
        profile.focusArea.includes('Algebra') ||
        profile.focusArea.includes('Geometry'));
    ...
    if (difficultyTier === 'Foundational') {
      ...
      if (isCorrect) {
        if (timeSpentSeconds <= targetTime) {
          ...
          masteryChange = 1;
          feedbackText = `Correct, but slow (${Math.round(
            timeSpentSeconds
          )}s). Target: ${targetTime}s`;
        }
      } else {
        masteryChange = -2;
      }
    ...
    if (profile) {
      const newMastery = Math.min(
        100,
        Math.max(0, profile.mastery + masteryChange)
      );
      ...
      <div className="max-w-md mx-auto mt-10 p-6 bg-white rounded-xl shadow-lg">
        ...
        >
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input
                  name="name"
                  required
                  className="w-full p-3 pl-10 border border-slate-200 rounded-lg outline-none focus:border-indigo-500 transition"
          ...
          <button
            ...
            className={`w-full mt-8 p-4 rounded-xl font-bold text-white transition flex items-center justify-center gap-2 ${
              isSubmitting
      ...
      <main className="flex-1 p-4">
        {mode === 'dashboard' && (
          <div className="space-y-6">
            <div className="bg-white p-6 rounded-2xl shadow-sm border border-slate-100 relative overflow-hidden">
              <div className="flex justify-between items-start mb-4">
                <div>
                  ...
                  <div className="text-lg font-black text-slate-800 leading-tight pr-2">
                    {profile?.focusArea}
                  </div>
                </div>
                <div
            ...
            <div className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden">
              ...
              <div className="max-h-40 overflow-y-auto">
                {profile?.weaknesses?.slice(0, 3).map((w, i) => (
                  <div
                    key={i}
                    className="p-3 border-b border-slate-50 last:border-0 flex items-center justify-between"
                  >
                    <span className="text-sm text-slate-600 truncate w-3/4">
                      {w}
                    </span>
            ...
            <button
              onClick={startDrill}
              className="w-full bg-slate-900 text-white p-4 rounded-xl shadow-lg active:scale-95 transition flex items-center justify-between group"
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  <div className="font-bold text-lg">Daily 5</div>
        ...
        {mode === 'drill' && (
          <div className="animate-in fade-in slide-in-from-bottom-4 duration-500 pb-20">
            {isGenerating ? (
              <Spinner text={`Generating ${difficultyTier} questions...`} />
            ) : activeQ ? (
              <div className="space-y-6">
                ...
                <div className="w-full bg-slate-200 h-2 rounded-full overflow-hidden">
                  <div
                    ...
                    style={{
                      width: `${
                        ((currentQIndex + 1) / quizQuestions.length) * 100
                      }%`,
                    }}
                  ></div>
                </div>
                ...
                {feedback && (
                  <div
                    className={`p-5 rounded-xl border animate-in fade-in slide-in-from-bottom-2 ${
                      feedback.isCorrect
                  ...
                  >
                    ...
                    <p className="text-slate-700 text-sm leading-relaxed mb-4">
                      {formatText(activeQ.explanation)}
                    </p>
                        ...
                        <div className="h-48 overflow-y-auto p-3 space-y-3 bg-white">
                          {chatMessages.length === 0 && (
                            <div className="text-center text-xs text-slate-400 mt-4">
                              ...
                              <button
                                onClick={() =>
                                  handleSendChat('Why is my answer wrong?')
                                }
                    ...
                    <button
                      ...
                      className="w-full mt-4 py-3 bg-slate-900 text-white rounded-xl font-bold hover:bg-slate-800 transition flex items-center justify-center gap-2"
                    >
              ...
              <div className="text-center p-10 text-red-500">
                Generation Error. Try refreshing.
              </div>
            )}
          </div>
        )}
          ...
          <div className="text-center py-10 space-y-6 animate-in zoom-in-95 duration-500">
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-slate-100">
              ...
              <p className="text-slate-600">
                You answered {sessionScore} out of {quizQuestions.length}{' '}
                correctly.
              </p>
            </div>
            <button
              onClick={() => setMode('dashboard')}
              className="w-full bg-indigo-600 text-white p-4 rounded-xl shadow-lg hover:bg-indigo-700 transition font-bold"
            >
              Return to Dashboard
            </button>
          </div>
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      fetchedStudents.sort(
        (a, b) => (b.lastActive?.toDate() || 0) - (a.lastActive?.toDate() || 0)
      );
        ...
        <div className="max-w-4xl mx-auto flex justify-between items-center">
          <div>
            <h1 className="text-2xl font-bold flex items-center gap-2">
              <ScanEye className="text-indigo-400" /> FOCUS PREP
            </h1>
          ...
          <div className="divide-y divide-slate-100">
            {students.map((student) => (
              ...
              >
                ...
                <button className="p-2 text-slate-400 hover:text-indigo-600 rounded-lg transition">
                  <AlertCircle size={20} />
                </button>
              </div>
            ))}
...
const App = () => {
        ...
        <div className="max-w-md w-full space-y-8">
          ...
          <div className="grid gap-4">
            <button
              ...
              className="group relative flex items-center p-6 bg-slate-800 rounded-2xl hover:bg-slate-700 transition border border-slate-700 hover:border-indigo-500"
            >
            ...
            <button
              ...
              className="group relative flex items-center p-6 bg-slate-800 rounded-2xl hover:bg-slate-700 transition border border-slate-700 hover:border-emerald-500"
            >
            ...
            <button
              ...
              className="group relative flex items-center p-6 bg-slate-800 rounded-2xl hover:bg-slate-700 transition border border-slate-700 hover:border-purple-500"
            >
              <div className="bg-purple-500/10 p-3 rounded-xl group-hover:bg-purple-500/20 transition">
              ...
              <div className="ml-4 text-left">
                ...
                <div className="text-slate-400 text-sm">Configure AI Logic</div>
              </div>
            </button>
          </div>
        </div>
  ...
  return role === 'student' ? (
    <StudentView user={user} onLogout={() => setRole(null)} />
  ) : role === 'counselor' ? (
    <CounselorView onLogout={() => setRole(null)} />
  ) : (
